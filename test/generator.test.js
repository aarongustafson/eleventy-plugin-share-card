...
describe('createGenerator — deferred mode (eleventyConfig provided)', () => {
	beforeEach(() => {
		lockMock.mockClear();
		sharpMock.mockClear();
		sharpCompositeMock.mockClear();
	...
	it('generates the image exactly once when eleventy.after fires', async () => {
		const { baseImagePath, outputDir, cacheFile } = makeTempPaths();
		const eleventyConfig = makeEleventyConfigMock();
	...
	it('does not regenerate an unchanged slug on a second eleventy.after (watch mode)', async () => {
		const { baseImagePath, outputDir, cacheFile } = makeTempPaths();
		const eleventyConfig = makeEleventyConfigMock();
	...
	it('returns empty string and warns when slug is missing (deferred mode)', async () => {
		const { baseImagePath, outputDir, cacheFile } = makeTempPaths();
		const eleventyConfig = makeEleventyConfigMock();
		const warnSpy = vi.spyOn(console, 'warn').mockImplementation(() => {});
